...
public class ExportComponent extends NotDraggableComponent{

	private static final long serialVersionUID = 1L;
	public ExportComponent(String componentName, int componentX,int componentY, int componentWidth, int componentHeight,boolean componentOut,int componentIndex,boolean isMenu) {
		super(componentName, componentX, componentY, componentWidth, componentHeight,componentIndex,isMenu);
    	this.ComponentType="ExportPY";
		setImage(false);
	 ...
	 @Override protected void setImage(boolean b){
		 String image;
		 if(b){image="exportPY_clicked.png";}
		 else{image="exportPY_not_clicked.png";}
	  ...
	  protected void inItem(boolean in){
		  if(in){
			  listener.changeIndication("Click to export the project to a .py file",0);
			  this.setImage(true);}
		  else{
			  this.setImage(false);
		  }
