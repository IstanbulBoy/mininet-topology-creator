...
public class DeleteComponent extends NotDraggableComponent{


	private static final long serialVersionUID = 1L;
	public DeleteComponent(String componentName, int componentX,int componentY, int componentWidth, int componentHeight,boolean componentOut,int componentIndex,boolean isMenu) {
		super(componentName, componentX, componentY, componentWidth, componentHeight,componentIndex,isMenu);
    	this.ComponentType="Delete component";
		setImage(false);
    	
	  addMouseListener(new MouseListener() {
	      @Override public void mouseClicked(MouseEvent e) {}
	      @Override public void mousePressed(MouseEvent e) {
	    	 listener.deleteComponent();
	 ...
	 @Override protected void setImage(boolean b){
		 String image;
		 if(b){image="eraser_clicked.png";}
		 else{image="eraser_not_clicked.png";}
	  ...
	  protected void inItem(boolean in){
		  if(in){
			  listener.changeIndication("Click to delete selected element",0);
			  this.setImage(true);}
		  else{
			  this.setImage(false);
		  }
