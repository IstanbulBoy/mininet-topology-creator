...
public class ImportComponent extends NotDraggableComponent{


	private static final long serialVersionUID = 1L;
	public ImportComponent(String componentName, int componentX,int componentY, int componentWidth, int componentHeight,boolean componentOut,int componentIndex,boolean isMenu) {
		super(componentName, componentX, componentY, componentWidth, componentHeight,componentIndex,isMenu);
    	this.ComponentType="Import";
		setImage(false);
	 ...
	 @Override protected void setImage(boolean b){
		 String image;
		 if(b){image="importXML_clicked.png";}
		 else{image="importXML_not_clicked.png";}
	  ...
	  protected void inItem(boolean in){
		  if(in){
			  listener.changeIndication("Click to import the project from a .xml file",0);
			  this.setImage(true);}
		  else{
			  this.setImage(false);
		  }
